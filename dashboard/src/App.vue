  ...
  <div class="shell">
    ...
    <main>
      ...
      <section v-if="section === 'overview'" class="view">
        ...
        <article class="panel integration-card"><div><h3>Google Calendar</h3><p v-if="googleCalendar.connected">Connected to {{ googleCalendar.calendarId }}</p><p v-else-if="googleCalendar.configured">Connect the test calendar before making real bookings.</p><p v-else>Add the Google Calendar values and encryption key to your local .env file.</p></div><button v-if="googleCalendar.configured && !googleCalendar.connected" class="primary" :disabled="busy" @click="connectGoogleCalendar">Connect Google Calendar</button><span v-else :class="['pill', { success: googleCalendar.connected }]">{{ googleCalendar.connected ? 'Connected' : 'Not configured' }}</span></article>
