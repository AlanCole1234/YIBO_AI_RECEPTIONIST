...
import { pcm16ToUlaw, RealtimeToUlawStream, TELEPHONE_SAMPLE_RATE, ulawToRealtimeFrame } from "../../../voice/index.js";
...
export interface AsteriskRtpVoiceMediaOptions {
  ...
  portStart: number;
  portEnd: number;
  /** Restricted to a specifically configured private test dialed number. */
  disableBargeInForTestDialedNumber?: string;
  /** Private, development-only tone source used to isolate the telephone leg. */
  audioDiagnostic?: { dialedNumber: string; source: "pcmu_tone" | "realtime_tone" };
...
export class AsteriskRtpVoiceMediaGateway implements VoiceMediaGateway {
  ...
  async prepare(callId: CallId, callerChannelId: string, dialedNumber?: string): Promise<void> {
    ...
    const disableBargeInForTest = Boolean(
      dialedNumber
      && this.options.disableBargeInForTestDialedNumber
      && dialedNumber === this.options.disableBargeInForTestDialedNumber,
    );
    const diagnostic = dialedNumber && this.options.audioDiagnostic?.dialedNumber === dialedNumber
      ? this.options.audioDiagnostic.source
      : undefined;
    const session = new AsteriskRtpSession(callId, callerChannelId, this.client, this.options, this.log, () => this.reservedPorts.delete(port), disableBargeInForTest, diagnostic);
  ...
  isDiagnosticCall(dialedNumber?: string): boolean {
    return Boolean(dialedNumber && this.options.audioDiagnostic?.dialedNumber === dialedNumber);
  }
...
class AsteriskRtpSession {
  ...
  constructor(
    ...
    private readonly disableBargeInForTest: boolean,
    private readonly diagnosticSource?: "pcmu_tone" | "realtime_tone",
  ) {
  ...
  transport(): ConversationTransport {
    return {
      ...
      ...(this.disableBargeInForTest ? { bargeInEnabled: false } : {}),
  ...
  private receiveRtp(packet: Buffer, remote: RemoteInfo): void {
    ...
    this.remote = remote;
    if (this.diagnosticSource) {
      this.startAudioDiagnostic();
      return;
    }
    try {
  ...
  private startAudioDiagnostic(): void {
    if (this.diagnosticStarted || !this.diagnosticSource) return;
    this.diagnosticStarted = true;
    const durationSeconds = 4;
    const source = this.diagnosticSource === "pcmu_tone"
      ? createPcmuTone(TELEPHONE_SAMPLE_RATE, durationSeconds)
      : new RealtimeToUlawStream().convert(createRealtimeTone(durationSeconds));
    this.log("telephony.media.audio_diagnostic_started", {
      callId: this.callId,
      source: this.diagnosticSource,
      durationSeconds,
      encodedBytes: source.byteLength,
    });
    this.pacer.enqueue(source, `diagnostic-${this.diagnosticSource}`);
  }
  ...
  private async sendRtpPacket(payload: Uint8Array, assistantTurnId: string, queueDepth: number, timing: RtpPacingTiming): Promise<void> {
    ...
    if (timing.talkspurtStarted) {
      ...
      const responseTiming = this.pendingResponseTiming;
      if (responseTiming) {
        const firstAudioToFirstRtpMs = this.firstRealtimeAudioReceivedAt === undefined
          ? undefined
          : Math.round(now - this.firstRealtimeAudioReceivedAt);
        this.log("telephony.turn.latency_summary", {
          callId: this.callId,
          turnNumber: responseTiming.turnNumber,
          ...(responseTiming.speechDurationMs === undefined ? {} : { speechDurationMs: responseTiming.speechDurationMs }),
          ...(responseTiming.configuredVadSilenceMs === undefined ? {} : { configuredVadSilenceMs: responseTiming.configuredVadSilenceMs }),
          // Server VAD reports its stop event after its configured silence
          // window. We intentionally expose that configuration rather than
          // pretending the transport can observe a caller's physical last
          // spoken sample.
          ...(responseTiming.configuredVadSilenceMs === undefined ? {} : { speechEndToVadStopMs: responseTiming.configuredVadSilenceMs }),
          ...(responseTiming.speechEndToCommitMs === undefined ? {} : { speechEndToCommitMs: responseTiming.speechEndToCommitMs }),
          ...(responseTiming.commitToDecisionMs === undefined ? {} : { commitToDecisionMs: responseTiming.commitToDecisionMs }),
          ...(responseTiming.toolDurationMs === undefined ? {} : { toolDurationMs: responseTiming.toolDurationMs }),
          ...(responseTiming.commitToResponseStartMs === undefined ? {} : { commitToResponseStartMs: responseTiming.commitToResponseStartMs }),
          responseStartToFirstAudioMs: responseTiming.responseStartToFirstAudioMs,
          ...(firstAudioToFirstRtpMs === undefined ? {} : { firstAudioToFirstRtpMs }),
          totalSpeechEndToAudibleReplyMs: responseTiming.totalSpeechEndToFirstAudioMs + (firstAudioToFirstRtpMs ?? 0),
          // This is the first packet handed to Asterisk. Carrier/device jitter
          // buffers are outside YIBO, so they are not represented as fake time.
          callerAudibleAfterFirstRtp: true,
          toolUsed: responseTiming.toolUsed ?? false,
          bargeInOccurred: responseTiming.bargeInOccurred ?? false,
          startingState: responseTiming.startingState,
          endingState: responseTiming.endingState,
        });
        this.pendingResponseTiming = undefined;
      }
    }
...
export class PcmuRtpPacer {
  ...
  private trimOverflow(): void {
    if (this.queuedBytes <= MAX_OUTBOUND_QUEUE_BYTES) return;
    const discarded = this.queuedBytes - MAX_OUTBOUND_QUEUE_BYTES;
    let remaining = discarded;
    while (remaining > 0 && this.queue.length) {
      const segment = this.queue[0]!;
      const available = segment.data.byteLength - segment.offset;
      const drop = Math.min(available, remaining);
      segment.offset += drop;
      remaining -= drop;
      this.queuedBytes -= drop;
      if (segment.offset === segment.data.byteLength) this.queue.shift();
    }
    this.options.log("telephony.media.rtp_queue_trimmed", {
      callId: this.options.callId,
      discardedBytes: discarded,
      discardedAudioMs: bytesToMilliseconds(discarded),
      queueDepth: this.queuedBytes,
      bufferedAudioMs: bytesToMilliseconds(this.queuedBytes),
    });
  }
...
function createPcmuTone(sampleRate: number, durationSeconds: number): Uint8Array {
  const samples = new Float32Array(sampleRate * durationSeconds);
  for (let index = 0; index < samples.length; index += 1) samples[index] = Math.sin((2 * Math.PI * 440 * index) / sampleRate) * 0.25;
  // Telephone test A intentionally starts at the PCMU boundary.
  const ulaw = new Uint8Array(samples.length);
  for (let index = 0; index < samples.length; index += 1) ulaw[index] = pcm16ToUlaw(Math.round((samples[index] ?? 0) * 0x7fff));
  return ulaw;
}

function createRealtimeTone(durationSeconds: number): AudioFrame {
  const sampleRate = 24_000;
  const samples = new Float32Array(sampleRate * durationSeconds);
  for (let index = 0; index < samples.length; index += 1) samples[index] = Math.sin((2 * Math.PI * 440 * index) / sampleRate) * 0.25;
  return createPcm16Frame(samples, sampleRate);
}

function createPcm16Frame(samples: Float32Array, sampleRate: number): AudioFrame {
  const data = new Uint8Array(samples.length * 2);
  const view = new DataView(data.buffer);
  for (let index = 0; index < samples.length; index += 1) view.setInt16(index * 2, Math.round(Math.max(-1, Math.min(1, samples[index] ?? 0)) * 0x7fff), true);
  return { data, codec: "pcm_s16le", sampleRate, channels: 1 };
}
...
class AsyncFrameQueue<T> implements AsyncIterable<T> {
  ...
  push(value: T): void {
    ...
    else if (!this.finished) this.values.push(value);
  }
